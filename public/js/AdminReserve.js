$('#student').ready(function() {
    $.ajax({
        ...
        success: function(response) {
            ...
            {
                if (parsedData[i].account_type == 'Student')
                {
                    $('#student').append($('<option value ="' + parsedData[i].display + '">' + parsedData[i].display + '</option>'));
                }
            }
        },
...
$('#view_button').click(function(event) {
    ...
    window.location.href = "/AdminIndex/" + $('#seatNo').val() + "/" + $('#reservationDate').val() + "/" + $('#reservationTime').val();
});
